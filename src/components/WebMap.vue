...
} from "../assets/timeRelatedFunctions";
      ...
      ([
            ...
            for (let i = 0; i < stations.length; i++) {
              ...
              layerList.listItemCreatedFunction = function (event) {
                ...
                if (item.title === "Stations") {
                  item.panel = {
                    content:
                      "<p>Stations rouges: observations et prévisions</p><p>Stations vertes: observations seulement",
                    open: true,
                  };
                }
              };
        ...
        let customContent = new CustomContent({
          ...
          creator: function (feature) {
            ...
            div.setAttribute("id", "popupGraph");
            dataPrediction = feature.graphic.attributes.predictions;

            div.innerHTML = "Prédictions <br>";

            //call fonction graph qui génère le graph d3js
            graph(dataPrediction, div, getDateValuesFromDate(date.dateChosen), timeSlider);
            return div;
          },
        });
        ...
        timeSlider.watch("values", function (values) {
          ...
          var elementExists = document.getElementById("popupGraph");
          if (elementExists) {
            //document.getElementById("popupGraph").innerHTML = "it works!";
            elementExists.innerHTML = "";
            graph(dataPrediction, elementExists, getDateValuesFromDate(date.dateChosen), timeSlider);
          }
